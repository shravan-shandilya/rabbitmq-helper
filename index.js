...
class RabbitMqProducer {
    constructor(options){
        if ((options == undefined) || (!options.ip) || (!options.queue)) {
            throw new Error('incorrect options to initialize RabbitMqProducer');
        }
        this.queue = options.queue;
        this.ip = options.ip;
        amqp.connect('amqp://' + options.ip)
        .then((conn) => {
    ...
    send(content){
        if (this.channel === undefined) {
            throw new Error('rabbitmqproducer not initialized, channel doesnot exist!');
        }
        let channelStatus = this.channel.sendToQueue(this.queue, new Buffer(JSON.stringify(content)), {
            persistent: true
        });
